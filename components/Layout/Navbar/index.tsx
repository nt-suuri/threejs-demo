  ...
  const navbar = (position: string) => (
    <>
      <div className="relative">
        <animated.div
          style={
            position == "absolute" ? defaultAnimatedProps : scrollAnimatedProps
          }
          className={
            position == "absolute"
              ? "absolute z-50 w-full top-0"
              : "z-50 w-full top-0 fixed"
          }
        >
          <nav className="flex items-center flex-wrap p-3 mr-6">
            ...
            <div className="z-40 inline-flex m-2 mt-1 mr-0 p-1 lg:hidden ml-auto outline-none bg-teal backdrop-blur-sm bg-opacity-20 rounded-xl">
              <Hamburger
                toggled={active}
                toggle={setActive}
                color={position == "fixed" ? "#edf5e1" : "#05396b"}
                distance="sm"
              />
            </div>
            <div
              className={` absolute lg:top-0 lg:relative top-24 ml-5 w-34 lg:inline-flex lg:flex-grow lg:w-auto`}
            >
              <animated.div
                style={{ ...animatedProps }}
                className="lg:ml-auto bg-teal backdrop-blur-sm bg-opacity-20 rounded-xl"
              >
                <div className="lg:inline-flex lg:flex-row lg:w-auto lg:items-center flex flex-col items-start lg:h-auto">
                  <Link href="/about">
                    <a
                      className={
                        (position == "fixed"
                          ? "text-white hover:text-pine"
                          : "text-navy hover:text-white") +
                        " text-xl lg:inline-flex lg:w-auto w-half px-4 py-2 rounded font-bold items-center justify-center"
                      }
                    >
                      About
                    </a>
